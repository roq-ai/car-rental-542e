...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getReservationsPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'reservations');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.reservations.count(countOptions as unknown),
  ...
  async function getReservations() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.reservations
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'reservations'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
