...
import { getReservations, deleteReservationsById } from 'apiSdk/reservations';
import { ReservationsInterface } from 'interfaces/reservations';

type ColumnType = ColumnDef<ReservationsInterface, unknown>;
...
export function ReservationsListPage(props: ReservationsListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ReservationsInterface>>(
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteReservationsById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ReservationsInterface) => {
    if (hasAccess('reservations', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              ...
              {hasAccess('reservations', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
              ...
              {hasAccess('reservations', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="reservations" />
        </Flex>

        {hasAccess('reservations', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
